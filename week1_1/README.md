...
### ● 11강 요약 (스택)
> * 자료를 보관할 수 있는 선형 구조
> * 단, 넣을때 한쪽끝으로 넣어야 하고(push), 꺼낼때는 한쪽 끝에서부터 꺼내야 함(pop)
> * LIFO (Last in first out) 구조를 가지는 자료구조
> * 대표적인 예시) 코드 함수 호출
> * 비어있는 스택에서 데이터를 꺼낼 때 = stack_underflow, 가득찬 스택에서 데이터를 넣을 때 = stack_overflow
> * 구현 방법으로는 Python 리스트 및 라이브러리 활용, 연결리스트를 이용하여 구현하는 방법이 있음 (양방향)
* 스택 연산의 정의
```
...
```
> * 스택 수식 괄호 유효성 검사.
> * 여는 괄호가 있으면 무조건 집어넣는다.
> * 닫는 괄호가 나오면, 스택이 차 있는지, 스택에서 데이터를 뽑아 쌍을 이루는지 확인해야한다.
> * 마지막에 스택이 비어있으면 정상적인 
